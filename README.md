...
> compatibility fEx: `docker build --platform=linux/arm64 --no-cache --tag trade-imports-ins-frontend`
...
```bash
docker run -p 3000:3000 trade-imports-ins-frontend:development
```
...
```bash
docker run -p 3000:3000 trade-imports-ins-frontend
```
